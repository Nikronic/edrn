...
from ipopt_helpers import initializeTensorProductSimulator  # type: ignore
...
morphology = 5  # TODO kernel size for now
rate = 0.7  # TODO
res_order = 'ftc'
repeat_res = 5  # TODO
epoch_mode = 'constant'
...
mrconfprint += 'Morphological filtering (opening->closing) with filter size: {}\n'.format(morphology)
...
# morphology
kernel = torch.rand((morphology, morphology)).float()
zero_tensor = torch.zeros((1, ))
if torch.cuda.is_available():
    kernel = kernel.cuda()
    kernel.requires_grad_(True)
    zero_tensor = zero_tensor.cuda()
sys.stderr.write('Kernel of morphological filters is: {}\n'.format('Trainable' if kernel.requires_grad else 'Fixed'))
if zero_tensor is not None:
    sys.stderr.write('Forcing positive weights on kernel with soft loss "maxed barrier"\n')
...
optim = torch.optim.Adam(lr=learning_rate, params=itertools.chain(list(model.parameters()), [kernel]), weight_decay=weight_decay)
# reduce on plateau
scheduler = None
if use_scheduler:
        ...
        for step in tqdm(range(epoch_sizes[idx]), desc='Training: '):
            ...
            def closure():
                ...
                if morphology is not None:
                    if step >= 200:
                        # force kernel to be positive
                        kernel_loss = 100 * torch.maximum(-torch.sum(kernel), zero_tensor) ** 2
                        density = density.unsqueeze(0).unsqueeze(0)
                        density = morph.close(tensor=morph.open(tensor=density, kernel=kernel), kernel=kernel)
