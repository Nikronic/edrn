...
adaptive_filtering_configs['sigma_init'] = 1  ###################### TODO
adaptive_filtering_configs['sigma_interval'] = 0.1  ################ TODO
adaptive_filtering_configs['sigma_scaler'] = -1  #################### TODO
...
if is_volume_constraint_satisfier_hard:
    ...
    nerf_model = MLP(in_features=2, out_features=1, n_neurons=256, n_layers=4, embedding_size=embedding_size,
                     scale=scale[0], hidden_act=nn.ReLU(), output_act=nn.ReLU())           
model = nerf_model
if torch.cuda.is_available():
    model.cuda()

# apply homogenization (almost garantees convergence to a acceptable solution)
...
for idx, res in enumerate(resolutions):
    for s in scale:
        ...
        SIMPExponent = 3
        constraints_smooth = [pyVoxelFEM.TotalVolumeConstraint(maxVolume)]
        uniformDensity = maxVolume
        tps_smooth = initializeTensorProductSimulator(
            orderFEM, domainCorners, gridDimensions, uniformDensity, E0, Emin, SIMPExponent, MATERIAL_PATH, BC_PATH)
        objective_smooth = pyVoxelFEM.ComplianceObjective(tps_smooth)
        top_smooth = pyVoxelFEM.TopologyOptimizationProblem(tps_smooth, objective_smooth, constraints_smooth, [])
        # instantiate autograd.Function for VoxelFEM engine
        voxelfem_engine_smooth = VoxelFEMFunction.apply

        SIMPExponent = 2
        constraints_pushed = [pyVoxelFEM.TotalVolumeConstraint(maxVolume)]
        tps_pushed = initializeTensorProductSimulator(
            orderFEM, domainCorners, gridDimensions, uniformDensity, E0, Emin, SIMPExponent, MATERIAL_PATH, BC_PATH)
        objective_pushed = pyVoxelFEM.ComplianceObjective(tps_pushed)
        top_pushed = pyVoxelFEM.TopologyOptimizationProblem(tps_pushed, objective_pushed, constraints_pushed, [])
        # instantiate autograd.Function for VoxelFEM engine
        voxelfem_engine_pushed = VoxelFEMFunction.apply
        ...
        for step in tqdm(range(epoch_sizes[idx]), desc='Training: '):
            ...
            def closure():
                ...
                if torch.cuda.is_available():
                    density = density.cpu()
                compliance_loss = torch.maximum(voxelfem_engine_smooth(density.flatten(), top_smooth),
                                                voxelfem_engine_pushed(density.flatten(), top_pushed))
                if torch.cuda.is_available():
        ...
        binary_compliance_loss = utils.compute_binary_compliance_loss(density=density, top=top_pushed,
                                                                      loss_engine=voxelfem_engine_pushed)
