...
if not fixed_scale:
    ...
    scale = [14.0]  # TODO
SIMPExponent = 3
resolutions = multires_utils.prepare_resolutions(interval=3, start=0, end=4, order=res_order, repeat_res=repeat_res)[:-1]  # TODO
epoch_sizes = multires_utils.prepare_epoch_sizes(n_resolutions=len(resolutions), # TODO
                                                 start=300, end=2500, 
                                                 mode=epoch_mode, constant_value=1000)
    ...
    for s in scale:
        ...
        if forgetting_weights is not None:
            multires_utils.forget_weights(model=model, rate=rate, mode=forgetting_weights,
                                            n_neurons=model.n_neurons, embedding_size=embedding_size)
                ...
                for i in range(len(model_inputs)):
                    ...
                    if not is_volume_constraint_satisfier_hard:
                        volume_loss = fem.satisfy_volume_constraint(density=density, max_volume=maxVolume,
                                                                    compliance_loss=single_res_compliance, 
                    ...
                    with torch.no_grad():
                        if (step+1) % (epoch_sizes[idx]) == 0:
                            ...
                            sys.stderr.write('Visualizing (woF) at iteration: {} for resolution: {}\n'.format(actual_steps, resolutions[i]))
                            ...
                            title +=   '_'+problem_name+'_Vol'+str(maxVolume_np)+'_F'+adaptive_filtering_configs_title
                            visualizations.density_vis(density, single_res_compliance.item(), resolutions[i], title, True, visualize, True,
