...
adaptive_filtering_configs['beta_init'] = 1  ##################### TODO
adaptive_filtering_configs['beta_interval'] = 0.1  ################## TODO
adaptive_filtering_configs['beta_scaler'] = -1  ################## TODO
...
log_base_path = 'logs/'
log_image_path = '{}images/issue63/'.format(log_base_path)
log_loss_path =  '{}loss/issue63/'.format(log_base_path)
log_weights_path =  '{}weights/issue63/'.format(log_base_path)
append_path = multires_utils.mkdir_multires_exp(log_image_path, log_loss_path, None, 
                                                experiment_id=args.jid)
log_image_path = '{}images/issue63/{}'.format(log_base_path, append_path)
log_loss_path =  '{}loss/issue63/{}'.format(log_base_path, append_path)
...
# learning_rate = 
optimizers = [torch_optimizer.AccSGD, 
              torch_optimizer.AdaBelief, torch_optimizer.AdaMod, torch_optimizer.Adafactor, 
              torch_optimizer.AdamP, torch_optimizer.Apollo, 
              torch_optimizer.DiffGrad, torch_optimizer.Lamb, torch_optimizer.NovoGrad, torch_optimizer.PID, 
              torch_optimizer.QHAdam, torch_optimizer.QHM, torch_optimizer.RAdam, torch_optimizer.RangerQH,
              torch_optimizer.SGDP, torch_optimizer.SGDW, torch_optimizer.SWATS, torch_optimizer.Shampoo]

sys.stderr.write('Using optimizers: {}\n'.format(optimizers))
sys.stderr.write('Global learning rate: {}\n'.format('default' if global_learning_rate is None else global_learning_rate))
...
for optim in optimizers:
    ...
    if global_learning_rate == 'lrx0.1':
        if optim.param_groups[0]['lr'] is not None:
            lr = optim.param_groups[0]['lr'] * 0.1
            optim.param_groups[0]['lr'] = lr
   
    sys.stderr.write('Using optimizer={}\n'.format(optim))
    ...
    for idx, res in enumerate(resolutions):
        ...
        for step in tqdm(range(epoch_sizes[idx]), desc='Training: '):
            ...
            def closure():
                ...
                if optim.__class__.__name__ == 'Adahessian':
                    compliance_loss.backward(create_graph=True)
                else:
        ...
        utils.save_weights(model, append_path[:-1]+optim_title if args.jid is None else args.jid+optim_title,
                          save_model, path=log_weights_path)
        ...
        title = 'FF('+optim_title+'_HC'+str(is_volume_constraint_satisfier_hard)+')_s'+str(scale)+'_'+forgetting_title+str(rate)+'_'+grid_title+'_'+str(idx+1)+'x'+str(actual_steps)
        ...
        if forgetting_activations is not None:
            title = 'FF('+optim_title+'_s'+str(scale)+'_'+forgetting_title+str(rate)+'_'+grid_title+'_'+str(idx+1)+'x'+str(actual_steps)
            title += problem_name+'_Vol'+str(maxVolume_np)
            visualizations.density_vis(density, compliance_loss_array_res[-1], gridDimensions, title, True, visualize, True,
                                       binary_loss=binary_compliance_loss, path=log_image_path)
        ...
        if forgetting_weights is not None:
            ...
            title = 'FF('+optim_title+'_HC'+str(is_volume_constraint_satisfier_hard)+'_max)_s'+str(scale)+'_'+forgetting_title+str(rate)+'_'+grid_title+'_'+str(idx+1)+'x'+str(actual_steps)
            title +=  problem_name+'_Vol'+str(maxVolume_np)
