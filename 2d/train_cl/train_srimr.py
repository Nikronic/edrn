...
if not fixed_scale:
    ...
    scale = [70.0]  # TODO
resolutions = multires_utils.prepare_resolutions(interval=5, start=0, end=11, order=res_order, repeat_res=repeat_res)[:-1]  # TODO
epoch_sizes = multires_utils.prepare_epoch_sizes(n_resolutions=len(resolutions),
                                                 start=800, end=1500, 
                                                 mode=epoch_mode, constant_value=1200)  # TODO
...
for idx, res in enumerate(resolutions):
    for s in scale:
        ...
        # Force sequential reinitialization by making sure model creates the same output at finer/coarser resolution
        sys.stderr.write('START --> forcing sequential reinitialization...\n')
        if idx > 0:
            # prepare previous ``res`` density of trained model 
            with torch.no_grad():
                gt_density = model(model_input)
                gt_density = gt_density.view(gridDimensions)
                # aka xPhys
                if is_volume_constraint_satisfier_hard:
                    gt_density = fem.satisfy_volume_constraint(density=gt_density, max_volume=maxVolume, compliance_loss=None,
                                                            mode=volume_constraint_satisfier, projection=projection_filter)
                else: 
                    gt_density = torch.clamp(gt_density, min=0., max=1.)
            # prepare previous ``res`` input data 
            ...
            gt_density = torch.nn.functional.interpolate(gt_density.unsqueeze(0).unsqueeze(0), align_corners=False,
                                                         size=gridDimensions, mode='bilinear').squeeze(0).squeeze(0)
            ...
            for ss in range(3000):
                ...
                supervised_loss = mse_crit(density, gt_density)
                
                optim.zero_grad()
                supervised_loss.backward()
                if (ss+1) % 250 == 0:
                    sys.stderr.write('Sequential Reinitialization Loss at {}: {}\n'.format(ss+1, supervised_loss.item()))
                optim.step()
            sys.stderr.write('END --> forcing sequential reinitialization...\n\n')
        ...
        title = 'FF(HC'+str(is_volume_constraint_satisfier_hard)+')_s'+str(scale)+'_'+grid_title+'_'+str(idx+1)+'x'+str(actual_steps)
...
sys.stderr.write('Query trained network for all training resolutions...\n\n')
for idx, res in enumerate(resolutions):
    gridDimensions = tuple(np.array(gridDimensions_) + res * np.array(domainCorners[1]))  # type: ignore
    test_resolution = gridDimensions
