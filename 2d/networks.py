...
# a simple MLP
class MLP(nn.Module):
    def __init__(self, in_features=2, out_features=1, n_neurons=256, n_layers=4, residuals=None,
        ...
        :param residuals: A list of indices indicating layers to be used as destination of residual connections
    ...
    def forward(self, coords):
        ...
        x_proj = torch.cat([torch.sin(x_proj), torch.cos(x_proj)], dim=-1)  # input.1

        if self.residuals is None:
            res = self.net(x_proj)
        else:  # [4, 8] only works for 6 layers and links to layer 2 and 4
            # pass
            res = self.net[0:2](x_proj)  # ...x256  # input.3
            x_proj = self.net[2:4+1](res)  # ...x256  # input.6
            res = self.net[4+1](x_proj + res)  # ...x265
            x_proj = self.net[4+2:8+1](res)  # ...x256
            res = self.net[8+1](x_proj + res)  # ...x256
            res = self.net[8+2:](res)  # ...x1
        return res
